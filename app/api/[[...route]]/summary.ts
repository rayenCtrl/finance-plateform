...
const app = new Hono()
    .get(
        ...
        async(c) => {
            ...
            const [currentPeriode] = await fetchFinancialData(
                auth.userId,
                startDate,
                endDate,
            );
            const [lastPeriode] = await fetchFinancialData(
                auth.userId,
                lastPeriodStart,
                lastPeriodEnd,
            );
            ...
            const incomeChange = calculatePercentageChange(
                currentPeriode.income,
                lastPeriode.income,
            );
            const expensesChange = calculatePercentageChange(
                currentPeriode.expense,
                lastPeriode.expense,
            );
            const remainingChange = calculatePercentageChange(
                currentPeriode.remaining,
                lastPeriode.remaining,
            );
            ...
            return c.json({
                data: {
                    remainingAmount: currentPeriode.remaining,
                    remainingChange,
                    incomeAmount: currentPeriode.income,
                    incomeChange,
                    expensesAmount: currentPeriode.expense,
                    expensesChange,
